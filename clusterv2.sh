...
aws ec2 describe-tags --region $REGION --filter "Name=resource-id,Values=$INSTANCE_ID" --query 'Tags[*].{Key:Key,Value:Value}' | jq -r '.[] | select( .Key as $a | ["Project", "Product","Project_owner","Schedule","researcher_name"] | index($a) )' >> out.json
